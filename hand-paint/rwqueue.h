...
#include "hand-paint/atomicops.h"
#include <new>
#include <type_traits>
#include <utility>
#include <cassert>
...
public:
	...
	AE_NO_TSAN explicit RWQueue(size_t size = 15)
#ifndef NDEBUG
		: enqueuing(false)
		,dequeuing(false)
#endif
	...
	AE_NO_TSAN RWQueue(RWQueue&& other)
	...
	// Note: The queue should not be accessed concurrently while it's
	// being moved. It's up to the user to synchronize this.
	RWQueue& operator=(RWQueue&& other) AE_NO_TSAN
	{
...
private:
	...
	// Disable copying
	RWQueue(RWQueue const&) {  }

	// Disable assignment
	RWQueue& operator=(RWQueue const&) {  }
...
// Like RWQueue, but also providees blocking operations
template<typename T, size_t MAX_BLOCK_SIZE = 512>
class BlockingRWQueue
{
private:
	typedef ::rwqueue::RWQueue<T, MAX_BLOCK_SIZE> RWQueue;
	
public:
	explicit BlockingRWQueue(size_t size = 15) AE_NO_TSAN
		: inner(size), sema(new spsc_sema::LightweightSemaphore())
	{ }

	BlockingRWQueue(BlockingRWQueue&& other) AE_NO_TSAN
		: inner(std::move(other.inner)), sema(std::move(other.sema))
	{ }

	BlockingRWQueue& operator=(BlockingRWQueue&& other) AE_NO_TSAN
	{
...
private:
	...
	BlockingRWQueue(BlockingRWQueue const&) {  }
	BlockingRWQueue& operator=(BlockingRWQueue const&) {  }
